...
Ext.ux.maximgb.treegrid.GridView = Ext.extend(Ext.grid.GridView, {
			...
			initTemplates : function() {
				...
				ts.master = new Ext.Template('<div class="x-grid3" hidefocus="true">', '<div class="x-grid3-viewport">', '<div class="x-grid3-header">',
						// Breadcrumbs
				...
				ts.row = new Ext.Template('<div class="x-grid3-row {alt} ux-maximgb-treegrid-level-{level}" style="{tstyle} {display_style}">',
						'<table class="x-grid3-row-table" border="0" cellspacing="0" cellpadding="0" style="{tstyle}">', '<tbody>', '<tr>{cells}</tr>', (this.enableRowBody
				...
				for (var j = 0, len = rs.length; j < len; j++) {
					...
					for (var i = 0; i < colCount; i++) {
						...
						if (c.id == this.grid.master_column_id) {
							...
							// modify by xlj 2010-9-3 解决ie6下内容超长后折行的问题。
							//（ie6下设置x-grid3-cell-inner层div的宽度小于column的宽度）
							if (Ext.isIE6) {
								var wrap_width = (ds.getNodeDepth(r) + 1) * 16;
								p.attr = 'style="padding-left:0px;width:' + (this.cm.getColumnWidth(i) - wrap_width - 8) + 'px;"';
							}
						} else {
							p.treeui = '';
						}
			...
			updateColumnWidth : function(col, width) {
				...
				for (var i = 0, len = ns.length; i < len; i++) {
					...
					// modify by xlj 2010-9-3 解决ie6下内容超长后折行的问题。
					//（ie6下column列宽改变时重新设置x-grid3-cell-inner层div的宽度）
					if (Ext.isIE6) {
						if (this.cm.getColumnId(col) == this.grid.master_column_id) {
							var wrap_width = (this.ds.getNodeDepth(this.ds.data.items[i]) + 1) * 16;
							this.getCell(i, col).childNodes[1].style.width = (this.cm.getColumnWidth(col) - wrap_width - 8) + 'px';
						}
					}
				}
