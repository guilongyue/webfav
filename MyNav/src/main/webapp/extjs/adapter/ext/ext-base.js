...
(function() {
	...
	var ua = navigator.userAgent.toLowerCase(), check = function(r) {
		...
		&& check(/version\/3/), isSafari4 = isSafari && check(/version\/4/), isIE = !isOpera && check(/msie/), isIE7 = isIE
		&& check(/msie 7/), isIE8 = isIE && check(/msie 8/), isIE6 = isIE && !isIE7 && !isIE8, isGecko = !isWebKit
		&& check(/gecko/), isGecko2 = isGecko && check(/rv:1\.8/), isGecko3 = isGecko && check(/rv:1\.9/), isBorderBox = isIE
	...
	Ext.apply(Ext, {
		 ...
		 * @param {Array/String} Either an Array of property names, or a comma-delimited list of property names to copy.
...
(function() {
	...
	Ext.lib.Ajax = {
		request : function(method, uri, cb, data, options) {
			...
			//加入参数 sync 处理同步请求
			return this[('sync' in options && options['sync'])?'syncRequest':'asyncRequest'](method, uri, cb, data);
		},
					...
					case 'select-multiple':
						for (var j = 0; j < el.options.length; j++) {
							if (el.options[j].selected) {
								var opt = el.options[j], sel = (opt.hasAttribute ? opt.hasAttribute('value') : opt
									.getAttributeNode('value').specified) ? opt.value : opt.text;
		...
		handleReadyState : function(o, callback) {
			var oConn = this;
			if (callback && callback.timeout && callback.timeout != 0) {//添加了callback.timeout != 0条件,表示当callback.timeout值为0时连接不超时   add by gh --2011,10,8;
	...
	Ext.lib.AnimBase.prototype = {
		...
		setRuntimeAttribute : function(attr) {
			...
			this.runtimeAttributes[attr].unit = (isset(attributes[attr].unit)) ? attributes[attr]['unit'] : this
				.getDefaultUnit(attr);
		},
			...
			var onTween = function() {
				...
				for (var attr in runtimeAttributes) {
					this.setAttribute(attr, this.doMethod(attr, runtimeAttributes[attr].start,
							runtimeAttributes[attr].end), runtimeAttributes[attr].unit);
				}
				actualFrames += 1;
			};
	...
	(function() {
		...
		proto.doMethod = function(attr, start, end) {
			var val = null;
			if (attr == 'scroll') {
				val = [this.method(this.currentFrame, start[0], end[0] - start[0], this.totalFrames),
