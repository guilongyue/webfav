...
Ext.DomHelper = function() {
	...
	var createDom = function(o, parentNode) {
		...
		if (Ext.isArray(o)) {
			...
			for (var attr in o) {
				if (attr == "tag" || attr == "children" || attr == "cn" || attr == "html" || attr == "style"
	...
	var ts = '<table>', te = '</table>', tbs = ts + '<tbody>', tbe = '</tbody>' + te, trs = tbs + '<tr>', tre = '</tr>'
		+ tbe;
	...
	return {
		...
		insertHtml : function(where, el, html) {
			...
			case "beforebegin":
				...
				//此处代码是为了解决EXTJS在IE9下面createContextualFragment方法报错的问题 WK-2012/02/01
...
Ext.EventManager = function() {
	...
	var pub = {
		...
		//供用户程序调用，手工触发resize事件，使得具有collapsible的panel发生窗口变化时通知对应的tabpanel，
		//tabpanel相应调整窗口。（tabpanel中需加入Ext.EventManager.onWindowResize监听）--add by xlj 2011-3-8
		fireAutoloadPanelResize : function() {
			curHeight = 0;
			curWidth = 0;
			if (resizeEvent) {
				resizeTask.delay(10);
	...
	var initExtCss = function() {
		...
		var cls = [
			...
			Ext.isIE ? "ext-ie " + (Ext.isIE6 ? 'ext-ie6' : (Ext.isIE7 ? 'ext-ie7' : 'ext-ie8')) : Ext.isGecko
				...
				: Ext.isOpera ? "ext-opera" : Ext.isSafari ? "ext-safari" : Ext.isChrome ? "ext-chrome" : ""];
		if (Ext.isMac) {
			cls.push("ext-mac");
		}
		if (Ext.isLinux) {
...
Ext.EventObject = function() {
	...
	Ext.EventObjectImpl.prototype = {
		...
		getTarget : function(selector, maxDepth, returnEl) {
			return selector ? Ext.fly(this.target).findParent(selector, maxDepth, returnEl) : (returnEl ? Ext
...
(function() {
	...
	El.prototype = {
		...
		anim : function(args, opt, animType, defaultDur, defaultEase, cb) {
			...
			opt = opt || {};
			var anim = Ext.lib.Anim[animType](this.dom, args, (opt.duration || defaultDur) || .35,
		...
		scrollIntoView : function(container, hscroll) {
			...
			var el = this.dom;
			var o = this.getOffsetsTo(c), l = o[0] + c.scrollLeft, t = o[1] + c.scrollTop, b = t + el.offsetHeight, r = l
				+ el.offsetWidth;
		...
		contains : function() {
			...
			return function(el) {
				return !this.dom.firstChild || // if this Element has no
					...
					!el || isXUL(el) ? false : Ext.lib.Dom.isAncestor(this.dom, el.dom ? el.dom : el);
			};
		}(),
		...
		setRegion : function(region, animate) {
			this.setBounds(region.left, region.top, region.right - region.left, region.bottom - region.top, this
					.preanim(arguments, 1));
			return this;
		},
		...
		getTextWidth : function(text, min, max) {
			return (Ext.util.TextMetrics.measure(this.dom, Ext.value(text, this.dom.innerHTML, true)).width).constrain(
				min || 0, max || 1000000);
		}
	};
	var ep = El.prototype;
...
Ext.Fx = {
	...
	fxUnwrap : function(wrap, pos, o) {
		...
		if (!o.wrap && wrap.dom.parentNode) {//在原来(!o.wrap)判断条件基础上增加了wrap.dom.parentNode不能为null的条件:为了解决在IE 下连续刷新时某些节点未被及时加载而出现wrap.dom.parentNode为空的错误;
	...
	request : function(o) {
		if (this.fireEvent("beforerequest", this, o) !== false) {
			...
			var cb = {
				...
				timeout : o.timeout == 0 ? o.timeout : o.timeout || this.timeout//将timeout:o.timeout || this.timeout修改为现在的形式,表示当callback.timeout值为0时连接不超时   modify by gh --2011,10,8;
			};
			...
			//add by xlj for get project id.						
			if (Ext.get('project_id') != null) {
				if (Ext.get('project_id').dom.value.length > 0) {
					url += (url.indexOf('?') != -1 ? '&' : '?') + "project_id=" + Ext.get('project_id').dom.value;
				}
			}
...
Ext.Updater = Ext.extend(Ext.util.Observable, {
	...
	startAutoRefresh : function(interval, url, params, callback, refreshNow) {
		...
		this.autoRefreshProcId = setInterval(this.update.createDelegate(this, [url || this.defaultUrl, params,
					callback, true]), interval * 1000);
	},
	...
	updateComplete : function(response) {
		...
		if (typeof response.argument.callback == "function") {
			response.argument.callback
				.call(response.argument.scope, this.el, true, response, response.argument.options);
		}
	},
	processFailure : function(response) {
		...
		if (typeof response.argument.callback == "function") {
			response.argument.callback.call(response.argument.scope, this.el, false, response,
...
Ext.Updater.defaults = {
	...
	indicatorText : '<div class="loading-indicator">加载中...</div>'
};
	...
	Ext.apply(Date, {
		...
		isValid : function(y, m, d, h, i, s, ms) {
			...
			return y == dt.getFullYear() && m == dt.getMonth() + 1 && d == dt.getDate() && h == dt.getHours()
				&& i == dt.getMinutes() && s == dt.getSeconds() && ms == dt.getMilliseconds();
		},
		...
		createParser : function() {
			var code = [
				...
				"v = new Date(y, m, d, h, i, s, ms);", "}", "}", "}", "if(v){", "if(zz != null){",
		...
		parseCodes : {
			...
			c : function() {
				var calc = [], arr = [$f("Y", 1), $f("m", 2), $f("d", 3), $f("h", 4), $f("i", 5), $f("s", 6), {
					c : "ms = results[7] || '0'; ms = parseInt(ms, 10)/Math.pow(10, ms.length - 3);\n"
				}, {
					c : ["if(results[8]) {", "if(results[8] == 'Z'){", "zz = 0;",
						"}else if (results[8].indexOf(':') > -1){", $f("P", 8).c, "}else{", $f("O", 8).c, "}", "}"]
						.join('\n')
				}];
...
Ext.apply(Date.prototype, {
	...
	getGMTOffset : function(colon) {
		return (this.getTimezoneOffset() > 0 ? "-" : "+")
			+ String.leftPad(Math.floor(Math.abs(this.getTimezoneOffset()) / 60), 2, "0") + (colon ? ":" : "")
			+ String.leftPad(Math.abs(this.getTimezoneOffset() % 60), 2, "0");
	},
	getDayOfYear : function() {
		var i = 0, num = 0, d = this.clone(), m = this.getMonth();
	...
	getWeekOfYear : function() {
		...
		return function() {
			var DC3 = Date.UTC(this.getFullYear(), this.getMonth(), this.getDate() + 3) / ms1d, AWN = Math.floor(DC3
				/ 7), Wyr = new Date(AWN * ms7d).getUTCFullYear();
...
Ext.util.Format = function() {
	...
	return {
		...
		dateRenderer : function(format) {
			return function(v) {
				// huzy 时间居左
				return jty.util.extTableRender(Ext.util.Format.date(v, format));
			};
		},
		stripTagsRE : /<\/?[^>]+>/gi,
...
Ext.XTemplate = function() {
	...
	while (m = s.match(re)) {
		...
		if (m3) {
			...
			if (exp) {
				fn = new Function('values', 'parent', 'xindex', 'xcount', 'with(values){ return '
		...
		if (m4) {
			...
			if (exp) {
				exec = new Function('values', 'parent', 'xindex', 'xcount', 'with(values){ '
						+ (Ext.util.Format.htmlDecode(exp)) + '; }');
			}
		}
		if (name) {
...
(function() {
	...
	Ext.dd.DragDrop.prototype = {
		...
		clickValidator : function(e) {
			...
			return (this.isValidHandleChild(target) && (this.id == this.handleElId || this.DDM.handleWasClicked(target,
				this.id)));
		},
...
Ext.data.Store = function(config) {
	...
	this.modified = [];
	this.addEvents('datachanged', 'metachange', 'add', 'remove', 'update', 'clear', 'beforeload', 'load',
		'loadexception');
	if (this.proxy) {
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
	...
	sortData : function(f, direction) {
		...
		var fn = function(r1, r2) {
			...
			if (v1 == null && v2 != null) {
				//modify by xlj 2012-5-11 解决非空字段和null比较不排序的问题。
				return -1;
			} else if (v1 != null && v2 == null) {
				//modify by xlj 2012-5-11 解决非空字段和null比较不排序的问题。
				return 1;
			} else if (typeof(v1) == 'string') {
				//modify by xlj 2012-5-11 为了适合汉字按拼音排序。
...
Ext.Component = function(config) {
	...
	this.addEvents('disable', 'enable', 'beforeshow', 'show', 'beforehide', 'hide', 'beforerender', 'render',
		'beforedestroy', 'destroy', 'beforestaterestore', 'staterestore', 'beforestatesave', 'statesave');
	this.getId();
...
Ext.extend(Ext.Component, Ext.util.Observable, {
	...
	getStateId : function() {
		return this.stateId
			|| ((this.id.indexOf('ext-comp-') == 0 || this.id.indexOf('ext-gen') == 0) ? null : this.id);
	},
	...
	getAutoCreate : function() {
		var cfg = typeof this.autoCreate == "object" ? this.autoCreate : Ext.apply({}, this.defaultAutoCreate);
		if (this.id && !cfg.id) {
			cfg.id = this.id;
		}
		return cfg;
...
Ext.BoxComponent = Ext.extend(Ext.Component, {
	...
	syncSize : function() {
		...
		//原来的程序在ie6下panel父窗口尺寸缩小后panel不能随之缩小。--modify by xlj 2011-3-8		
		this.setSize(this.autoWidth ? undefined : (Ext.isIE6 ? this.el.dom.parentElement.clientWidth : this.el
				.getWidth()), this.autoHeight ? undefined : (Ext.isIE6
				? this.el.dom.parentElement.clientHeight
				: this.el.getHeight()));
		return this;
	},
...
Ext.extend(Ext.SplitBar, Ext.util.Observable, {
	onStartProxyDrag : function(x, y) {
		...
		if (this.orientation == Ext.SplitBar.HORIZONTAL) {
			...
			this.dd.setXConstraint(this.placement == Ext.SplitBar.LEFT ? c1 : c2, this.placement == Ext.SplitBar.LEFT
					? c2
					: c1);
			...
			this.dd.setYConstraint(this.placement == Ext.SplitBar.TOP ? c1 : c2, this.placement == Ext.SplitBar.TOP
					? c2
					: c1);
		}
...
Ext.layout.FitLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	onLayout : function(ct, target) {
		...
		if (!this.container.collapsed) {
			var sz = (Ext.isIE6 && Ext.isStrict && target.dom == document.body) ? target.getViewSize() : target
				.getStyleSize();
			this.setItemSize(this.activeItem || ct.items.itemAt(0), sz);
		}
	},
	setItemSize : function(item, size) {
		//modify by xlj 2010-7-15 解决firefox下width=0，不显示的问题。
		//if (item && size.height > 0) {
		if (item && size.height > 0 && size.width > 0) {
			item.setSize(size);
		}
	}
});
	...
	onLayout : function(ct, target) {
		...
		if (!this.rendered) {
			...
			for (var i = 0, len = items.length; i < len; i++) {
				...
				this[pos] = pos != 'center' && c.split ? new Ext.layout.BorderLayout.SplitRegion(this, c.initialConfig,
					pos) : new Ext.layout.BorderLayout.Region(this, c.initialConfig, pos);
...
Ext.layout.BorderLayout.Region = function(layout, config, pos) {
	...
	if (this.collapsible) {
		...
		if (this.collapseMode == 'mini' && !this.cmargins) {
			...
			this.cmargins = Ext.applyIf(this.cmargins || {}, pos == 'north' || pos == 'south'
...
Ext.layout.BorderLayout.Region.prototype = {
	...
	beforeExpand : function(animate) {
		...
		this.el.show();
		//为了解决expand时触发两次resize事件的问题。这样在界面自适应时，只监听resize事件，而且不会触发多次。
		//另外解决了this.panel在折叠后宽度不对的问题。（ie下每次折叠少2px，firefox下第二次折叠后宽度为0）modify by xlj 2011-3-8
		//		if (this.position == 'east' || this.position == 'west') {
		//			this.panel.setSize(undefined, c.getHeight());
		//		} else {
		//			this.panel.setSize(c.getWidth(), undefined);
		//		}
		delete this.panel.lastSize;
		c.hide();
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {
	...
	setContainer : function(ct) {
		...
		if (!this.fieldTpl) {
			var t = new Ext.Template('<div class="x-form-item {5}" tabIndex="-1">',
				'<label for="{0}" style="{2}" class="x-form-item-label">{1}{4}</label>',
				'<div class="x-form-element" id="x-form-el-{0}" style="{3}">', '</div><div class="{6}"></div>',
				'</div>');
	...
	renderItem : function(c, position, target) {
		if (c && !c.rendered && c.isFormField && c.inputType != 'hidden') {
			var args = [c.id, c.fieldLabel, this.getLabelStyle(c.labelStyle), this.elementStyle || '',
...
Ext.layout.AbsoluteLayout = Ext.extend(Ext.layout.AnchorLayout, {
	...
	getAnchorViewSize : function(ct, target) {
		return this.isForm ? ct.body.getStyleSize() : Ext.layout.AbsoluteLayout.superclass.getAnchorViewSize.call(this,
			ct, target);
	},
	isValidParent : function(c, target) {
...
Ext.Panel = Ext.extend(Ext.Container, {
	...
	initComponent : function() {
		...
		if (this.autoloadResize) {
			//界面自适应。注意：panel中要设置autoloadResize:true,autoWidth:true -add by xlj 2011-3-9
			Ext.EventManager.onWindowResize(this.fireAutoloadResize, this);
		}
	},
	//需执行的界面自适应操作。 -add by xlj 2011-3-9
	fireAutoloadResize : function() {
		//该panel未隐藏时才进行syncsize操作。
		if (this.el) {
			if (this.el.dom.offsetWidth > 0 || this.el.dom.offsetHeight > 0) {
				this.syncSize();
			}
		}
	},
	...
	onCollapse : function(doAnim, animArg) {
		if (doAnim) {
			this[this.collapseEl].slideOut(this.slideAnchor, Ext.apply(this.createEffect(animArg || true,
						this.afterCollapse, this), this.collapseDefaults));
		} else {
	...
	onExpand : function(doAnim, animArg) {
		if (doAnim) {
			this[this.collapseEl].slideIn(this.slideAnchor, Ext.apply(this.createEffect(animArg || true,
						this.afterExpand, this), this.expandDefaults));
		} else {
	...
	doAutoLoad : function() {
		...
		u.update(typeof this.autoLoad == 'object' ? this.autoLoad : {
			url : this.autoLoad
		});
	},
	getTool : function(id) {
...
Ext.extend(Ext.ColorPalette, Ext.Component, {
	...
	colors : ["000000", "993300", "333300", "003300", "003366", "000080", "333399", "333333", "800000", "FF6600",
		"808000", "008000", "008080", "0000FF", "666699", "808080", "FF0000", "FF9900", "99CC00", "339966", "33CCCC",
		"3366FF", "800080", "969696", "FF00FF", "FFCC00", "FFFF00", "00FF00", "00FFFF", "00CCFF", "993366", "C0C0C0",
...
Ext.DatePicker = Ext.extend(Ext.Component, {
	...
	onRender : function(container, position) {
		...
		m.push('</tr></tbody></table></td></tr>', this.showToday
	...
	createMonthPicker : function() {
		if (!this.monthPicker.dom.firstChild) {
			...
			buf.push('<tr class="x-date-mp-btns"><td colspan="4"><button type="button" class="x-date-mp-ok">',
				this.okText, '</button><button type="button" class="x-date-mp-cancel">', this.cancelText,
				'</button></td></tr>', '</table>');
	...
	update : function(date, forceRefresh) {
		...
		if (this.showToday) {
			...
			var disable = (td < min || td > max || (ddMatch && format && ddMatch.test(td.dateFormat(format))) || (ddays && ddays
				.indexOf(td.getDay()) != -1));
			if (!this.disabled) {
	...
	beforeDestroy : function() {
		if (this.rendered) {
			...
			this.keyNav = null;
			Ext
				.destroy(this.leftClickRpt, this.rightClickRpt, this.monthPicker, this.eventEl, this.mbtn,
					this.todayBtn);
		}
	}
});
...
Ext.Button = Ext.extend(Ext.Component, {
	...
	initComponent : function() {
		...
		this.addEvents("click", "toggle", 'mouseover', 'mouseout', 'menushow', 'menuhide', 'menutriggerover',
			'menutriggerout');
		if (this.menu) {
	...
	initButtonEl : function(btn, btnEl) {
		...
		if (this.repeat) {
			var repeater = new Ext.util.ClickRepeater(btn, typeof this.repeat == "object" ? this.repeat : {});
	...
	onDisableChange : function(disabled) {
		if (this.el) {
			...
			//原来不可用 改成可用
			/*
			 * if(this.disabled && !disabled){ grayscale.reset(this.el.dom); }else if(disabled){//原来可用 改为不可用
			 * //grayscale.prepare(this.el.dom); grayscale(this.el.dom); }
			 */
...
(function() {
	...
	// liuzhixin 为分隔符添加id by 2013-5-15 11:31
	Ext.reg('tbitem', T.Item);
	T.Separator = function() {
		var s = document.createElement("span");
		var id;
		if (arguments.length > 0 && (id = arguments[0]['id'])) {
			s.id = id;
		}
		s.className = "ytb-sep";
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
	...
	lastText : "Last Page",
	refreshText : "Refresh",
	selectFlag : "0",//扩展该属性是用于判断是否调用,store.reload(callback:function ());中的callback方法,此处采用字符串作为值是为了以后在其他地方扩展使用,默认是"0",可设值其他值,"RS",已经被用;
	paramNames : {
		start : 'start',
		limit : 'limit'
	},
	...
	updateInfo : function() {
		if (this.displayEl) {
			...
			var msg = count == 0 ? this.emptyMsg : String.format(this.displayMsg, this.cursor + 1, this.cursor + count,
	...
	doLoad : function(start) {
		...
		var flag = this.selectFlag;//此处须先把值付给flag再在callback中使用;
		if (this.fireEvent('beforechange', this, o) !== false) {
			this.store.load({
				params : o,
				callback : function(r, options, success) {
					if (flag == "RS") {
						jty.common.selectGridPanelRows(r);//在人员选择界面的人员搜索标签也中在翻页的时候选中当前页中原本已经选中的记录;
					}
				}
			});
		}
	...
	startSizing : function(e, handle) {
		...
		if (this.enabled) {
			...
			if (this.constrainTo) {
				...
				this.resizeRegion = ct.getRegion().adjust(ct.getFrameWidth('t'), ct.getFrameWidth('l'),
...
Ext.MessageBox = function() {
	...
	return {
		...
		updateText : function(text) {
			...
			var w = Math.max(Math.min(opt.width || iw + mw + fw + bw, this.maxWidth), Math.max(opt.minWidth
			...
			if (Ext.isIE6) {//huzy 2010-05-04 对于ie6浏览器 对话框宽度放10象素
				w += 10;
			}
		...
		show : function(options) {
			...
			//add by xlj 2010-8-24 setting messagebox‘s z-index
			//为messagebox.show的options增加zindex参数。
			if (opt.zindex) {
				d.setZIndex(opt.zindex);
			}
			d.on('show', function() {
				if (allowClose === true) {
					d.keyMap.enable();
		...
		//add by xlj 2011-4-6 为message.alert中增加zindex，设置zindex，以免被其它部分遮挡。
		alert : function(title, msg, fn, scope, zindex) {
			this.show({
				title : title,
				msg : msg,
				buttons : this.OK,
...
Ext.tree.TreePanel = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		this.addEvents("append", "remove", "movenode", "insert", "beforeappend", "beforeremove", "beforemovenode",
			"beforeinsert", "beforeload", "load", "textchange", "beforeexpandnode", "beforecollapsenode", "expandnode",
			"disabledchange", "collapsenode", "beforeclick", "click", "checkchange", "dblclick", "contextmenu",
...
Ext.extend(Ext.tree.TreeNode, Ext.data.Node, {
	...
	getLoader : function() {
		var owner;
		return this.loader
			|| ((owner = this.getOwnerTree()) && owner.loader ? owner.loader : new Ext.tree.TreeLoader());
	},
	setFirstChild : function(node) {
	...
	expandChildNodes : function(deep) {
		var cs = this.childNodes;
		if (cs && cs.length) {//在原来没有判断基础上增加了(cs && cs.length)的判断条件:为了解决在IE 下连续刷新时某些节点未被及时加载而出现cs.length为空的错误,此问题和下面注释中的wrap.dom.parentNode为空的错误为一个问题;
			for (var i = 0, len = cs.length; i < len; i++) {
				cs[i].expand(deep);
			}
		}
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		//modify by xlj 2010-7-23 解决firefox下触发window.beforeunload事件的问题。
		...
		var buf = ['<li class="x-tree-node"><div ext:tree-node-id="', n.id,
			...
			'<span class="x-tree-node-indent">', this.indentMarkup, "</span>", '<img src="', this.emptyIcon,
			'" class="x-tree-ec-icon x-tree-elbow" />', '<img src="', a.icon || this.emptyIcon,
...
Ext.extend(Ext.tree.TreeLoader, Ext.util.Observable, {
	...
	processResponse : function(response, node, callback) {
		...
		try {
			var o = eval("(" + json + ")");
			//在FireFox(2.6.3)有些低版本浏览器中当o的值为空的时候不能自动抛出异常，需要手动抛出异常;
			if (!o && Ext.isGecko)
				throw new Error("在json对象为空的时候抛出异常！");
			//未加载完即关闭或切换tree，可能会提示：length为空或不是对象
			//判断node.childNodes来屏蔽该错误。add by xlj 2010-12-29
...
Ext.extend(Ext.tree.TreeEditor, Ext.Editor, {
	...
	fitToTree : function(ed, el) {
		...
		var w = Math.min(this.maxWidth, (td.clientWidth > 20 ? td.clientWidth : td.offsetWidth)
...
Ext.extend(Ext.menu.Menu, Ext.util.Observable, {
	...
	autoWidth : function() {
		...
		if (w) {
			...
			//参照ExtJs3.0对此处做了修改，以解决IE8兼容模式下日期显示过窄的问题。 --modify by xlj 2014-6-13
			el.setWidth(Ext.isStrict && (Ext.isIE7 || Ext.isIE8) ? 'auto' : this.minWidth);
...
Ext.extend(Ext.menu.Item, Ext.menu.BaseItem, {
	...
	onRender : function(container, position) {
		...
		el.innerHTML = String.format('<img src="{0}" class="x-menu-item-icon {2}" />{1}', this.icon
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {
	...
	selectOnFocus : false,
	//add by xlj 2011-4-7 startupFocus用于判断该域在创建后是否自动获得焦点，一般一个form中最好只有一个field的startupFocus设置成true。
	startupFocus : false,
	...
	initEvents : function() {
		...
		if (this.maskRe
			|| (this.vtype && this.disableKeyFilter !== true && (this.maskRe = Ext.form.VTypes[this.vtype + 'Mask']))) {
...
Ext.form.NumberField = Ext.extend(Ext.form.TextField, {
	...
	setValue : function(v) {//huzy ExtJS NumberField小数自动截最后0问题，不显示最后0问题
		v = typeof v == 'number' ? v : parseFloat(String(v).replace(this.decimalSeparator, "."));
		//v = isNaN(v) ? '' : v.toFixed(this.decimalPrecision).replace(".",this.decimalSeparator);
		//modify by xlj 2010-7-19 原语句没有加入allowDecimals参数的判断。
		v = isNaN(v) ? '' : v.toFixed(this.allowDecimals ? this.decimalPrecision : 0).replace(".",
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
		...
		if (this.transform) {
			...
			if (!this.store) {
				...
				for (var i = 0, len = opts.length; i < len; i++) {
					var o = opts[i], value = (o.hasAttribute
						? o.hasAttribute('value')
						: o.getAttributeNode('value').specified) ? o.value : o.text;
					if (o.selected) {
						this.value = value;
					}
					d.push([value, o.text]);
	...
	onDestroy : function() {
		...
		this.bindStore(null);
		Ext.destroy(this.resizer, this.view, this.pageTb, this.innerList, this.list);
	...
	fireKey : function(e) {
		//只在keydown或keypress时触发，不在keyup时触发，否则会执行两次。-modify by xlj 2012-6-27 
		if (((Ext.isIE && e.type == 'keydown') || e.type == 'keypress') && e.isSpecialKey()) {
			var fn = function(ev) {
	...
	beforeBlur : function() {
		// 好像有bug，隐藏该功能。 modify by xlj
		/*
		 * var val = this.getRawValue(); if (this.forceSelection) { if (val.length > 0 && val != this.emptyText) {
		 * this.el.dom.value = this.lastSelectionText === undefined ? '' : this.lastSelectionText;
		 * this.applyEmptyText(); } else { this.clearValue(); } } else { var rec = this.findRecord(this.displayField,
		 * val); if (rec) { val = rec.get(this.valueField || this.displayField); } this.setValue(val); }
		 */
		//修改空值失去焦点不起作用  2013-1-10 by lzx
		this.el.blur();
	},
...
//把extjs插件中的fileuploadfield.js融入到ext.js,不再用单独的插件文件了。--add by xileijiang 2014-12-22
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
	...
	enableLinks : true,
	enableFont : true,
	createLinkText : '请输入连接地址：',
	defaultLinkValue : 'http:/' + '/',
	fontFamilies : ['黑体', '宋体', 'Arial', 'Courier New', 'Tahoma', 'Times New Roman', 'Verdana'], // modify by xlj
	defaultFont : '宋体',// modify by xlj
	...
	createFontOptions : function() {
		...
		for (var i = 0, len = fs.length; i < len; i++) {
			ff = fs[i];
			lc = ff.toLowerCase();
			buf.push('<option value="', lc, '" style="font-family:', ff, ';"', (this.defaultFont == lc
					? ' selected="true">'
					: '>'), ff, '</option>');
		}
		return buf.join('');
	...
	createToolbar : function(editor) {
		...
		if (this.enableFontSize) {
			tb.add('-', btn('increasefontsize', false, this.adjustFont),
	...
	pushValue : function() {
		if (this.initialized) {
			...
			if (this.fireEvent('beforepush', this, v) !== false) {
				...
				if (Ext.isGecko) {
					var d = this.doc, mode = d.designMode.toLowerCase();
...
Ext.form.TimeField = Ext.extend(Ext.form.ComboBox, {
	...
	invalidText : "{0} \u4e0d\u662f\u6b63\u786e\u7684\u65f6\u95f4",//将英文提示改为中文
	format : "g:i A",
...
Ext.form.Action.prototype = {
	...
	createCallback : function(opts) {
		...
		return {
			...
			scope : this,
			timeout : opts.timeout * 1000,//原来代码：(opts.timeout * 1000) || (this.form.timeout * 1000)
...
Ext.form.VTypes = function() {
	...
	var email = /^(\w+)([-+.][\w]+)*@(\w[-\w]*\.){1,5}([A-Za-z0-9]){1,4}$/;//Extjs原来的邮箱验证:email = /^(\w+)([-+.][\w]+)*@(\w[-\w]*\.){1,5}([A-Za-z]){2,4}$/;修改成@后面可以是IP地址或域均能通过验证;
...
Ext.grid.GridPanel = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		this.addEvents("click", "dblclick", "contextmenu", "mousedown", "mouseup", "mouseover", "mouseout", "keypress",
			"keydown", "cellmousedown", "rowmousedown", "headermousedown", "cellclick", "celldblclick", "rowclick",
			"rowdblclick", "headerclick", "headerdblclick", "rowcontextmenu", "cellcontextmenu", "headercontextmenu",
			"bodyscroll", "columnresize", "columnmove", "sortchange");
	},
	onRender : function(ct, position) {
		...
		// 为了统一GridPanel行的左右键选中方式。add by xlj 2010-3-5
	...
	onRowClick : function(grid, rowIndex, e) {
		if (grid._status == 'right') {
			grid.getView().removeRowClass(grid._row, 'ux-grid-row-rightselected');
			grid._status = 'left';
		} else {
			grid._status = 'left';
		}
	...
	onRowContextMenu : function(grid, rowIndex, e) {
		...
		//胡赵勇 2010年5月26日16:51:52 当表格没有选择器的时候 grid.getSelectionModel().selectRow方法会没有的
	...
	getDragDropText : function() {
		...
		return "您选择了 " + count + " 条记录";// huzy 拖拽显示乱码问题的解决
	}
});
...
Ext.grid.GridView = function(config) {
	Ext.apply(this, config);
	this.addEvents("beforerowremoved", "beforerowsinserted", "beforerefresh", "rowremoved", "rowsinserted",
		"rowupdated", "refresh");
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	sortAscText : "\u5347\u5e8f", //升序
	sortDescText : "\u964d\u5e8f", // 降序
	saveColumnStyleText : "\u4fdd\u5b58",//保存列表样式
	cancelColumnStyleText : "\u53d6\u6d88",//取消保存样式列表
	columnsText : "\u5217",//列
	borderWidth : 2,
	...
	initTemplates : function() {
		...
		if (!ts.master) {
			//增加x-selectable样式，使得在firefox、chrome可以选中grid中的内容进行复制。-add by xlj 2012-12-14
			ts.master = new Ext.Template(
				'<div class="x-grid3" hidefocus="true">',
				'<div class="x-grid3-viewport">',
				'<div class="x-grid3-header"><div class="x-grid3-header-inner"><div class="x-grid3-header-offset" style="{ostyle}">{header}</div></div><div class="x-clear"></div></div>',
				'<div class="x-grid3-scroller"><div class="x-grid3-body x-selectable" style="{bstyle}">{body}</div><a href="#" class="x-grid3-focus" tabIndex="-1"></a></div>',
				'</div>', '<div class="x-grid3-resize-marker">&#160;</div>',
				'<div class="x-grid3-resize-proxy">&#160;</div>', '</div>');
		}
		if (!ts.header) {
			ts.header = new Ext.Template('<table border="0" cellspacing="0" cellpadding="0" style="{tstyle}">',
		...
		if (!ts.row) {
			ts.row = new Ext.Template(
				...
				'<tbody><tr>{cells}</tr>',
				(this.enableRowBody
					? '<tr class="x-grid3-row-body-tr" style="{bodyStyle}"><td colspan="{cols}" class="x-grid3-body-cell" tabIndex="0" hidefocus="on"><div class="x-grid3-row-body cke_htmlstyle">{body}</div></td></tr>'
					: ''), '</tbody></table></div>');
		}
		if (!ts.cell) {
	...
	getEditorParent : function() {//胡赵勇 2010年5月26日17:05:35 在靠近滚动条的可编辑组件 双击 只会出现一半  或更少
		//原代码 return this.scroller.dom;
	...
	scrollToTop : function() {
		if (this.scroller.dom.scrollTop) {
			this.scroller.dom.scrollTop = 0;
		}
		//重新加载store中的数据后，grid不再横向滚动到最左边。--modify by xlj 2013-2-1
		//if (this.scroller.dom.scrollLeft) {
	...
	renderUI : function() {
		...
		if (g.enableHdMenu !== false) {
			...
			this.hmenu.add({
				id : "saveColSty",
				text : this.saveColumnStyleText,
				icon : 'images/save.gif'
			}, {
				id : "cancelColSty",
				text : this.cancelColumnStyleText,
				icon : 'images/business/update_reset.gif'
			});
			if (g.enableColumnHide !== false) {
				this.hmenu.add('-', {
					id : "columns",
	...
	resolveCell : function(row, col, hscroll) {
		...
		var rowEl = this.getRow(row), cm = this.cm, colCount = cm.getColumnCount(), cellEl;
	...
	getColumnWidth : function(col) {
		...
		if (typeof w == 'number') {
			return (Ext.isBorderBox || (Ext.isWebKit && !Ext.isSafari2) ? w : (w - this.borderWidth > 0 ? w
				- this.borderWidth : 0))
				+ 'px';
		}
		return w;
	...
	destroy : function() {
		...
		Ext.destroy(this.resizeMarker, this.resizeProxy, this.focusEl, this.mainBody, this.scroller, this.mainHd,
			this.mainWrap, this.dragZone, this.splitZone, this.columnDrag, this.columnDrop);
	...
	handleHdMenuClick : function(item) {
		...
		case "desc":
			...
			break;
		case "saveColSty":
			jty.common.columnStatus(this.grid);
			break;
		case "cancelColSty":
			jty.common.resetColumnStatus(this.grid);
			break;
		default:
	...
	beforeColMenuShow : function() {
		...
		var reg = /^.*style=.*$/i;//验证列名中是否含有"style=''"这种形式;
		var filterReg = /(style\s*=\s*[\"\'](\\\'|\\\"|[^\"\'])*[\"\'])/g;//过滤列名中的样式正则;

		for (var i = 0; i < colCount; i++) {
			if (cm.config[i].fixed !== true && cm.config[i].hideable !== false) {
				var columnHeader = cm.getColumnHeader(i);

				//过滤形如：header:"<span style="width:100%;">11</span>"表头html标签中的style样式:过滤之后为：header:"<span>11</span>"
				if (reg.test(columnHeader)) {//如果列名中含有样式则过滤;
					columnHeader = columnHeader.replace(filterReg, "");
				}
				//此处加入判断，在列显示菜单中，不创建序号列的选择框,也同时解决了现实undefined的问题
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	initTemplates : function() {
		...
		if (!this.startGroup) {
			this.startGroup = new Ext.XTemplate('<div id="{groupId}" class="x-grid-group {cls}">',
				'<div id="{groupId}-hd" class="x-grid-group-hd" style="{style}"><div>', this.groupTextTpl,
	...
	doRender : function(cs, rs, ds, startRow, colCount, stripe) {
		...
		for (i = 0, len = rs.length; i < len; i++) {
			...
			var r = rs[i], gvalue = r.data[groupField], g = this.getGroup(gvalue, r, groupRenderer, rowIndex, colIndex,
				ds);
		...
		for (i = 0, len = groups.length; i < len; i++) {
			...
			buf[buf.length] = Ext.grid.GroupingView.superclass.doRender.call(this, cs, g.rs, ds, g.startRow, colCount,
				stripe);
...
Ext.extend(Ext.grid.GridDragZone, Ext.dd.DragZone, {
	...
	getDragData : function(e) {
		...
		if (rowIndex !== false) {
			...
			// modify by xlj for resolution to dragdrop&rowclick conflict
...
Ext.extend(Ext.grid.RowSelectionModel, Ext.grid.AbstractSelectionModel, {
	...
	initEvents : function() {
		...
		} else { // allow click to work like normal add by xlj for resolution
			// to dragdrop&rowclick conflict
			this.grid.on("rowclick", function(grid, rowIndex, e) {
				var target = e.getTarget();
				if (target.className !== 'x-grid3-row-checker' && e.button === 0) {
					this.handleMouseDown(grid, rowIndex, e);
					// this.selectRow(rowIndex, e.shiftKey || e.ctrlKey);
...
Ext.extend(Ext.grid.CellSelectionModel, Ext.grid.AbstractSelectionModel, {
	initEvents : function() {
		...
		this.grid.getGridEl().on(Ext.isIE || (Ext.isWebKit && !Ext.isSafari2) ? "keydown" : "keypress",
...
Ext.extend(Ext.grid.GridEditor, Ext.Editor, {
	...
	shim : false,
	shadow : false,
	startEdit : function(el, value) { //此处是从Editor.js中拷贝过来的
		if (this.editing) {
			this.completeEdit();
		}
		this.boundEl = Ext.get(el);
		...
		this.startValue = v;
		this.field.record = this.record; // 这句话是新加的，用于在处理EDITOR里面的值时，可以直接拿到当前GRID的当前记录RECORD
		this.field.setValue(v);
		this.doAutoSize();
		this.el.alignTo(this.boundEl, this.alignment);
		this.editing = true;
...
Ext.grid.RowNumberer.prototype = {
	...
	renderer : function(v, p, record, rowIndex) {
		...
		// 为了解决grid翻页时序号递增。--add by xlj 2010-3-5
		if (record.store.lastOptions != undefined && record.store.lastOptions.params !== undefined) {
			if (record.store.lastOptions.params.start !== undefined) {
...
Ext.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
	...
	initEvents : function() {
		...
		this.on('selectionchange', this.autoAllCheckedGridHead, this);
	},
	...
	autoAllCheckedGridHead : function() {//根据是否全选来自动选中表头;
		if (this.grid) {
			var view = this.grid.getView();
			var hd = Ext.fly(view.innerHd);
			var c = hd.query(".x-grid3-hd-checker");
			var head = c[0];
			if (head != null) {
				var extHead = Ext.fly(head);
				if (this.grid.getSelectionModel().getSelections().length != this.grid.getStore().getCount()) {
					//清空表格头的checkbox
					if (extHead.hasClass("x-grid3-hd-checker-on")) {
						extHead.removeClass("x-grid3-hd-checker-on");
					}
				} else {
					if (this.grid.getStore().getCount() == 0)
						return;
					extHead.addClass("x-grid3-hd-checker-on");
				}
			}
		}
	}
...
Ext.ProgressBar = Ext.extend(Ext.BoxComponent, {
	...
	onRender : function(ct, position) {
		...
		var tpl = new Ext.Template('<div class="{cls}-wrap">', '<div class="{cls}-inner">', '<div class="{cls}-bar">',
...
Ext.debug.DomTree = Ext.extend(Ext.tree.TreePanel, {
	...
	initComponent : function() {
		...
		function onNodeSelect(t, n, last) {
			...
			if (n && n.htmlNode) {
				...
				if (styles && !showAll.pressed) {
					...
					for (var a in dom) {
						var v = dom[a];
						if ((isNaN(a + 10)) && v != undefined && v !== null && v !== ''
		...
		hnode = this.root.appendChild(new Ext.debug.HtmlNode(document.getElementsByTagName('html')[0]));
	}
});
